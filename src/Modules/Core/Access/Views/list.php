...
<div class="app-main__inner">
        ...
        <div class="page-title-wrapper">
            <div class="page-title-heading">
                <div class="page-title-icon">
                    <i class="pe-7s-unlock icon-gradient bg-strong-bliss"></i>
                </div>
                <div>Access</b>
                    <div class="page-title-subheading">Daftar Semua Access</div>
                </div>
            </div>
            <div class="page-title-actions">
                <nav class="" aria-label="breadcrumb">
                    <ol class="breadcrumb">
                        ...
                        <li class="active breadcrumb-item" aria-current="page">Accesss</li>
                    </ol>
                </nav>
            </div>
        </div>
    ...
    <div class="container">
        <ul class="tabs-animated-shadow tabs-animated nav nav-justified tabs-rounded-lg">
            ...
            <li class="nav-item">
                <a role="tab" class="nav-link <?=($row->id == $group->id )?'active':''?>" href="<?=base_url('access?group_id='.$row->id)?>" aria-selected="true">
                    <span><?=$row->name?></span>
                </a>
            </li>
            <?php endforeach; ?>
                                        ...
                                        <div class="widget-content-left mr-3">
                                            <div class="switch has-switch switch-container-class" data-class="fixed-setting">
                                                <div class="switch-animate switch-on">
                                                    <input type="checkbox" class="module_checkbox check" name="permissions[]" 
                                                    value="<?= strtolower($row->controller . '/' . $permission) ?>" 
                                                    data-perm='<?= strtolower($row->controller . '/' . $permission) ?>' 
                                                    data-id='<?= $row->id ?>' data-module='<?= strtolower($row->controller); ?>' 
                                                    data-permission='<?= strtolower($permission); ?>' id='cb_<?= $kk . $k ?>' 
                                                    data-toggle="toggle" data-onstyle="success" <?php if (in_array(strtolower($row->controller) . '/' . strtolower($permission), $access)) echo 'checked'; ?>>
                                                </div>
                                            </div>
                                        </div>
                                            ...
                                            <div class="widget-content p-0">
                                                <div class="widget-content-wrapper">
                                                    <div class="widget-content-left mr-3">
                                                        <div class="switch has-switch switch-container-class" data-class="fixed-setting">
                                                            <div class="switch-animate switch-on">
                                                                <input type="checkbox" class="module_checkbox" name="permissions[]" value="<?= strtolower($row2->controller . '/' . $permission2) ?>" data-perm='<?= strtolower($row2->controller . '/' . $permission2) ?>' data-id='<?= strtolower($row2->id); ?>' data-module='<?= strtolower($row2->controller); ?>' data-permission='<?= strtolower($permission2); ?>' id='cb_<?= $kk2 . $k2 ?>' data-toggle="toggle" data-onstyle="success" <?php if (in_array(strtolower($row2->controller) . '/' . strtolower($permission2), $access)) echo 'checked'; ?>>
                                                            </div>
                                                        </div>
                                                    </div>
                                                    <div class="widget-content-left">
