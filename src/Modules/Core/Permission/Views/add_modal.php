        ...
        <div class="modal-content">
            <div class="modal-header">
                <h5 class="modal-title">
                    <i class="header-icon lnr-plus-circle icon-gradient bg-plum-plate"> </i> <?= lang('Permission.btn.create'); ?>
                </h5>
            ...
            <form id="frm_create" method="post" action="">
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        ...
                        <div>
                            <input type="text" class="form-control" id="name" name="name" placeholder="<?= lang('Permission.label.name') ?>" value="<?= set_value('name'); ?>" />
                        </div>
                    </div>
                    <div class="form-group">
                        <label for="description"><?= lang('Permission.label.description') ?></label>
                        <div>
                            <textarea id="description" name="description" placeholder="<?= lang('Permission.label.description') ?>" rows="1" class="form-control autosize-input" style="min-height: 38px;"><?= set_value('group_description'); ?></textarea>
                        </div>
                    </div>
                </div>
        ...
        $.ajax({
            ...
            .done(function(res) {
                ...
                if (res.status === 200) {
                    Swal.fire({
                        title: 'Success',
                        text: '<?= lang('Permission.info.success.create') ?>',
                        type: 'success',
